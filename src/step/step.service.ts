...
export class StepService {
  private loger = new Logger(StepService.name);

  constructor(
    @InjectRepository(Step) private readonly step: Repository<Step>,
  ) {}
  ...
  async create(username: string, password: string, step: string) {
    try {
      // 先测试账号密码是否正确
      const flag = await this.getStep(username, password, step);
      this.loger.log(flag);
      if (flag === '登录失败' || !flag) {
        return false;
      }
  ...
  @Cron('0 0 18 * * *')
  async autoGetStep() {
    const data = await this.step.find();
    data.forEach((item) => {
      this.getStep(item.account, item.password, item.step);
    });
  }

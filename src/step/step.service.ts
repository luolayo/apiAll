...
import { createIp, get, post } from '../util/request';
import { getStepData, loginData, TimeStampData, tokenData } from './types';
...
export class StepService {
  ...
  async getStep(username: string, password: string, step: string) {
    ...
    const data = await post<getStepData>(
      'getStep',
  ...
  private async getToken(login_token: string) {
    const res = await get<tokenData>(
      'getToken',
      `https://account-cn.huami.com/v1/client/app_tokens?
      app_name=com.xiaomi.hm.health&dn=api-user.huami
      .com%2Capi-mifit.huami.com%2Capp-analytics.huami.com
      &login_token=${login_token}&os_version=6.3.5`,
    );
  ...
  private async getTimeStamp(): Promise<string> {
    const res = await get<TimeStampData>(
      'getTimeStamp',

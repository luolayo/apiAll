...
export class StepService {
  ...
  private async getCode(username: string, password: string) {
    ...
    const res = await this.request(
      `https://api-user.huami.com/registrations/${userinfo}/tokens`,
      'POST',
      {
        client_id: 'HuaMi',
        country_code: 'CN',
  ...
  private async getToken(login_token: string) {
    const res = await this.request(
      `https://account-cn.huami.com/v1/client/app_tokens?app_name=com.xiaomi.hm.health&dn=api-user.huami.com%2Capi-mifit.huami.com%2Capp-analytics.huami.com&login_token=${login_token}&os_version=6.3.5`,
      'GET',
      {},
      {
        'x-forwarded-for': this.createIp(),
  ...
  private getJsonData(step: string) {
    ...
    return decodeURIComponent(
      data_json
        .toString()
        .replace(
          /(date%22%3A%22).*?(%22%2C%22data)/g,
          `$1${dayjs().format('YYYY-MM-DD')}$2`,
        )
